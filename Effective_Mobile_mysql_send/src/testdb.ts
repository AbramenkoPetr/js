...
import { MySqlDataSource } from "./MsqlDataSource";
  ...
  .then(async connection => {
     console.log("Data Source has been initialized successfully.");
        ...
        app.post('/list-of-requests', async (req, res) => {
                ...
                ms = req_body.ms;
                ys = req_body.ys;
                if((ds== undefined||ms== undefined||ys == undefined)||(ds== ''||ms== ''||ys == '')) ds=ms=ys='';
                date_start = ys+"-"+ms+"-"+ds;
        ...
        app.get('/job-all-cancel', async (req, res) => {
              ithem1 = await connection
              .createQueryBuilder()
              .update<Requests>(Requests, { status: "Отменено", reason_for_cancellation: "Групповая отмена" })
